<h1>Udacity-Facebook Secure & Private AI</h1>
<p>Notebooks associated with the Udacity-Facebook Secure &amp; Private AI Challenge</p>

<h2>Below is the archive of my #60DaysofUdacity updates</h2>

<p><strong>Encouragement buddies for #60daysofUdacity</strong></p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 1/60 (6/27/2019)</h3>
<hr>
<h4>Step 1:</h4>  
<p>pledge made 6/27/2019</p> 
<h4>Step 2:</h4>  
<ul>
  <li>Worked my way through Lesson 5, videos 7 and 8 and associated notebook exercises.</li>
  <li>Project T-Shirt initiated!</li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>  
<h4>Step 4:</h4> 
<p>Shout-out to my #60daysofUdacity friends! You got this!</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
</ul>

<br>
<h3>Day 2/60 (6/28/2019)</h3>
<hr>
<h4>Step 1:</h4> 
<p>Pledge made 6/27/2019</p>  
<h4>Step 2:</h4> 
<ul>
  <li>Finished Lesson 5 and associated notebook exercises. Started Lesson 6.</li>
  <li>Progress continues on Project T-Shirt. Timeline due for team review 7/4.</li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this!</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 3/60 (6/29/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>   
<ul>
  <li>Worked through Video 3 of Lesson 6.</li>
  <li>Progress continues on Project T-Shirt:   
    <ul>
      <li>researching t-shirt printers</li>
      <li>organizing notebook concepts for students to participate</li>
      <li>choosing platform for file sharing.</li>
      <li>Timeline due 7/4 for team review.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome! </li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 4/60 (6/30/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Worked through Video 4 of Lesson 6.</li>
  <li>Progress continues on Project T-Shirt:   
    <ul>
      <li>Learned about workflow tools available in GitHub.</li>
      <li>Timeline due 7/4 for team review.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 5/60 (7/1/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>   
<ul>
  <li>Worked through Video 6 of Lesson 6.</li>
  <li>Lesson 6 final project: Developed individual private models on which to perform the differential privacy and PATE analysis. Might do some more later tonight.</li>
  <li>Progress continues on Project T-Shirt:
    <ul>
      <li>Decided on TeeSpring as the shirt printer.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome! </li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 6/60 (7/2/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Continued work on Lesson 6 final project.</li>
  <li>Progress continues on Project T-Shirt. 
    <ul>
      <li>Sent survey out to Secure and Private AI cohort.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome! </li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>  
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>
  
<br>
<h3>Day 7/60 (7/3/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>  
<h4>Step 2:</h4>   
<ul>
  <li>Continued work on Lesson 6 final project. 
    <ul>
      <li>Broke the training dataset into 6 subsets.</li>
      <li>Trained a model on each subset.</li>
      <li>Created a list aggregating all of the predictions from each model.</li>
      <li>Will start working on the DP component (arg_max and PATE) tonight or tomorrow.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt.   
    <ul>
      <li>Sent survey out to Secure and Private AI cohort. Responses due 7/9. Will upload timeline to GitHub.</li>
      <li>Requested channel for the project.</li> 
      <li>Will send @akshit.udacity a write-up to include in a future digest of activities.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 8/60 (7/4/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Continued work on Lesson 6 final project. Made it through the code, but seeing some weird results from data-dependent vs. data-independent epsilon. Would appreciate any advice on where to start looking. (See #l5_diff_privacy_dl)</li>
  <li>Progress continues on Project T-Shirt.  
    <ul>
      <li>@LauraT organized a study group.</li> 
      <li>Uploaded a first draft of a calendar for the rest of the project.</li> 
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 9/60 (7/5/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Continued work on Lesson 6 final project.
    <ul>
      <li>Reworking my final project for Lesson 5.</li>
      <li>Shoutout to @Diego and and @Archit for helping me through some modeling challenges around data-dependent and data-independent epsilons.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt.  
    <ul>
      <li>Study group is live!</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 10/60 (7/6/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Finished Lesson 6.</li>
  <li>Progress continues on Project T-Shirt. 
    <ul>
      <li>Shared an overview deck with the team for review.</li>
      <li>Created a photo album in our Facebook Group for image submissions (Group Home Page > Photos > Albums > Project T-Shirt).</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome! </li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 11/60 (7/7/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Started Lesson 7: Federated Learning.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Published the slides announcing our project: <a href="https://docs.google.com/presentation/d/e/2PACX-1vSRrQJpf_U2eRt3iwu2HxiSSkKrCwlfiYWbnVDmXxDxju1R2BN8f8NMbxpTr1QM9e4quKQW_Zahu1gE/pub?start=false&loop=false&delayms=3000&slide=id.g5d0203f5a3_0_88">#60DaysofUdacity Project T-Shirt</a></li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
</ul>
<h4>Step 3:</h4>
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 12/60 (7/8/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Continued Lesson 7: Federated Learning.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Discussing plans for kick-off event on 7/12/2019.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 13/60 (7/9/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Continued Lesson 7: Federated Learning: building linear model.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Discussing plans for kick-off event on 7/12/2019.</li>
      <li>Contacted @akshit.udacity for details on how to schedule a YouTube session that includes both livestream and screen sharing.</li>
      <li>Closed Project T-Shirt survey regarding types of swag, charity to receive donations, etc.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 14/60 (7/10/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Continued Lesson 7: Federated Learning: building linear model.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Discussing plans for kick-off event on 7/12/2019.</li>
      <li>Figuring out how to live stream on YouTube.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Passed the AZ-900 Fundamentals of Azure certification exam!</li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 15/60 (7/11/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: No progress today.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Prepared for kick-off event on 7/12/2019.
        <ul>
          <li>Activated YouTube account and scheduled LiveStream event for 12:00 PM EST on Friday, July 12. <a href="https://www.youtube.com/watch?v=LVRhGK1lG98">Link here</a></li>
          <li>Organized Skype contacts in advance so that we can stream the call over YouTube.</li>
          <li>Configured OBS Studio encoder to broadcast the Skype call</li>
          <li>Created <a href="https://docs.google.com/presentation/d/e/2PACX-1vRs-TDE4z31UX-LxrhFRsZ4mrMmW3xs6cLRd_5YaufKvlkiASyfwJTS3Jjsfq53eA/pub?start=false&loop=false&delayms=3000">flyer</a> to advertise the event.</li>
        </ul>
      </li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>In the process of evacuating for Tropical Storm/Hurricane Barry.</li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 16/60 (7/12/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: No progress today.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Hosted kick-off event on 7/12/2019.
        <ul>
          <li>Experienced some livestreaming technical difficulties, most likely due to bandwidth and PC.</li>
          <li>Recorded conversation with core team and will post to YouTube 7/13/2019.</li>
          <li>HUGE thank you to @KT for an excellent walk-through of her GAN project notebook. I highly recommend checking it out once posted!</li>
        </ul>
      </li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Finished evacuating for Tropical Storm/Hurricane Barry.</li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 17/60 (7/13/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: Successfully ran linear model on virtual worker. Watched a few more videos.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Still working on compiling the kickoff video to post to YouTube. Trying on a different laptop.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 18/60 (7/14/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: No progress.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Successfully published the kickoff video to YouTube: <a href="https://youtu.be/8_vhbNpyIk4">Watch here</a>.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 19/60 (7/15/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: 
    <ul>
      <li>Watched videos through 12: Final Project Description</li>
      <li>Did prep work for the final project: figuring out which dataset to use, reviewing resources classmates have recommended on #l6_federated_learning, etc. Thanks to @Ajay Singh and @Archit for pointing me in the right direction!</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 20/60 (7/16/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: 
    <ul>
      <li>Started working on Final Project. Running into a few issues (see below). I am using Google Colab. I imported the OpenMined notebook associated with <a href="https://blog.openmined.org/upgrade-to-federated-learning-in-10-lines/">DEEP LEARNING -> FEDERATED LEARNING IN 10 LINES OF PYTORCH + PYSYFT</a> into Colab and saw the same issues with the sample notebook.
        <ul>
          <li>I can't move the model from the CPU to the GPU.</li>
          <li>I receive the following message when I attempt to run the test method: <i>"_pickle.PicklingError: Can't pickle <class 'syft.frameworks.torch.tensors.interpreters.native.Tensor'>: attribute lookup Tensor on syft.frameworks.torch.tensors.interpreters.native failed
"</i>. </li>
        </ul>
      </li>
      <li>Posted my issues in #l6_federated_learning. Any and all advice would be greatly appreciated.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 21/60 (7/17/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: 
    <ul>
      <li>Substantial progress on final project:
        <ul>
          <li>Contacted the OpenMined folks about my issues reported on Day 20/60.</li>
          <li>Learned that PySyft currently cannot support GPU.</li>
          <li>I had removed all references to the GPU except **kwargs, which I referenced in the definitions of my federated_trainloader and testloader. After I removed those arguments, the model trained without a problem.</li>
          <li>Asked the team in #l6_federated_learning for help understanding if disaggregated gradients come back to the local machine if using the federated trainloader approach to training a model remotely.</li>
        </ul>
      </li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 22/60 (7/18/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: 
    <ul>
      <li>Made final aesthetic edits to final project. Explained what I used from the OpenMined blog. </li>
      <li>Much thanks to @Saumya for helping me get my head wrapped around federated dataloaders in PySyft.</li>
      <li>Watched Guest Interview: Federated Learning at Google.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Starting to see some really great images post in the Facebook Group!</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 23/60 (7/19/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 7: Federated Learning: 
    <ul>
      <li>Rewatched Guest Interview: Federated Learning at Google.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Starting to see some really great images post in the Facebook Group!</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>


<br>
<h3>Day 24/60 (7/20/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 8: Securing Federated Learning: 
    <ul>
      <li>Revising final project from Lesson 7.</li>
      <li>Huge thank-you to @Edgar Villegas for posting his GitHub repo in #60DaysofUdacity. (https://secureprivataischolar.slack.com/archives/CKRQP9ZLL/p1563448157249400) I was having a hard time wrapping my head around training the models individually on the different workers and found your method very informative.</li>
      <li>Need to get my head wrapped around deconstructing weights and biases in the individual models so that I can add them together.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Uploaded a few images to the Facebook Group for a participant without access to Facebook. </li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>


<br>
<h3>Day 25/60 (7/21/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 8: Securing Federated Learning: 
    <ul>
      <li>Revising final project from Lesson 7.</li>
      <li>Made really good progress on adding the aggregator worker to the final project for Lesson 7 (start of Lesson 8).</li>
      <li>Shout out to @Alison Davey for directing me for a great aggregator example: https://github.com/OpenMined/PySyft/blob/dev/examples/tutorials/Part%2010%20-%20Federated%20Learning%20with%20Secure%20Aggregation.ipynb</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 26/60 (7/22/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 8: Securing Federated Learning: 
    <ul>
      <li>Finished version of Lesson 7 final project that uses trusted aggregator. Ties into first assignment of Lesson 8.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Created skeleton of publicly available image gallery for those participants who cannot access Facebook: https://ewotawa.github.io/spaic_tshirt_gallery/</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 27/60 (7/23/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 8: Securing Federated Learning: 
    <ul>
      <li>Started the videos for Lesson 8.</li>
      <li>Set up Section 3 notebook.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Launched publicly available image gallery for voting and for those participants who cannot access Facebook: https://ewotawa.github.io/spaic_tshirt_gallery/</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 28/60 (7/24/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 8: Securing Federated Learning: 
    <ul>
      <li>Finished the videos for Lesson 8.</li>
      <li>Prepped for Lesson 8 final project.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Set up the TeeSpring page to host the swag for Project T-Shirt: https://teespring.com/stores/udacity-spaic</li>
      <li>Decided to abandon the charitable donation element of Project T-Shirt due to tax implications. Merchandise will be sold at cost instead.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 29/60 (7/25/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 8: Securing Federated Learning: 
    <ul>
      <li>Completed for Lesson 8 final project.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Created a few style transfer images for Project T-Shirt.</li>
      <li>Updated the public gallery for new images posted to Facebook.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 30/60 (7/26/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Starting watching videos.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Updated the public gallery for new images posted to Facebook.</li>
      <li>Reminded participants to use only images in the public domain.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 31/60 (7/27/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>No progress today.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Updated the public gallery for new images posted to Facebook and to #sg_project-tshirt.</li>
      <li>Chatted with the team regarding images with recognizable characters. Not including in voting out of abundance of caution around copyright law. Working on good faith that the students are following our instruction to use open copyright images for their source features and styles.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 32/60 (7/28/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Watched another video.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Updated the public gallery for new images posted to Facebook and to #sg_project-tshirt.</li>
      <li>Got updates from participants about images' public domain status. Updated gallery accordingly.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 33/60 (7/29/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Watched another video.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 34/60 (7/30/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>No progress today.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Updated the public gallery with new images. We now have over 200 works created by students using AI!</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 35/60 (7/31/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>No progress today.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Updated the public gallery with new images. We now have over 300 works created by students using AI!</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 36/60 (8/1/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>No progress today.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>A few gallery updates:
        <ul>
          <li>I sorted the images by student/team. Standardized naming conventions where there was some variation.</li>
          <li>I turned on @Abhishek Lalwani's Iron Man images, after we confirmed that the image was in the public domain.</li>
          <li>I corrected the inadvertent duplication/swap of one of @Rishi S Rao’s images.</li>
          <li>Turned on a Kung Fu Panda image from team unstoppable that I had inadvertently turned off.</li>
        </ul>
      </li>
      <li>Next gallery steps:
        <ul>
          <li>I'll work on consolidating @Suparna S Nair's combinations of image and shirt design. Probably won't get to this one until late Friday.</li>
        </ul>
      </li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 37/60 (8/2/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>No progress today. Might watch a video or two later tonight.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>A few gallery updates:
        <ul>
          <li>Added a "design" field to the page so that students who submitted a shirt design along with their images could display both as a single entry (@Suparna S Nair).</li>
          <li>Corrected two inadvertent duplicates in @Evi's submissions.</li>
          <li>In the process of confirming the identity of the images submitted under the name "Seeratpal Kaur".</li>
        </ul>
      </li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 38/60 (8/3/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>No progress today. Might watch a video or two later tonight.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>A few gallery updates:
        <ul>
          <li>Added inadvertently missing images for @Seeratpal K. Jaura, @Abhishek Tandon, @Munira Omar</li>
          <li>Corrected attributions for @Seeratpal K. Jaura</li>
          <li>Turned on the two images team unstoppable made using a Facebook logo that was from Unsplash (public domain image).</li>
        </ul>
      </li>
      <li>Handed off to @Sarah Majors to open voting.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 39/60 (8/4/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Started working on Project Demo - Build an Encrypted Database</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 40/60 (8/5/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Continued working on Project Demo - Build an Encrypted Database</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 41/60 (8/6/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Continued working on Project Demo - Build an Encrypted Database</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 42/60 (8/7/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Finished watching on Project Demo - Build an Encrypted Database</li>
      <li>Ran into an error when trying to mimic the instructor's code in Colab. Posted the question to #l8_encrypted_dl: https://secureprivataischolar.slack.com/archives/CJVD619MG/p1565232988093400</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 43/60 (8/8/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Lesson 9: Encrypted Deep Learning: 
    <ul>
      <li>Continued watching videos.</li>
      <li>Much thanks to @sourav kumar and @José Fonceca for helping me with my question. I also posted the question in the OpenMined Slack community. Will share any information I get back from the PySyft team. See #l8_encrypted_dl: https://secureprivataischolar.slack.com/archives/CJVD619MG/p1565232988093400</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 44/60 (8/9/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Finished the videos for the course!</li>
  <li>Coming up with a game plan for the final project.</li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 45/60 (8/10/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Figured out a way to add model parameters together on differential privacy exercise.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Gearing up for work in TeeSpring. Thanks to @Yaak for leading the charge on prepping images and to @Sarah Majors for organizing the vote!</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 46/60 (8/11/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Continued to tinker with workbook.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Prepping the TeeSpring store.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 47/60 (8/12/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Continued to tinker with workbook.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>After researching the source of the winning image, the team elected not to print it on merchandise out of an abundance of caution around copyright issues.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 48/60 (8/13/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Continued to tinker with workbook.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Continued to prep the TeeSpring site.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 49/60 (8/14/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Stalled on workbook until I have a better wifi connection.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Continued to prep the TeeSpring site.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 50/60 (8/16/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Took 8/15 off due to work travel.</li>
  <li>Final Project:
    <ul>
      <li>Continued working with notebook.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Continued to prep the TeeSpring site.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 51/60 (8/17/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Got base image classifier working.</li>
      <li>Migrated project from Google Colab to Jupyter notebooks on Anaconda.</li>
      <li>Researching how to get PyTorch to recognize my NVIDIA GPU. Doing what I can with CUDA.</li>
      <li>Prepping second phase of notebook to apply federated learning techniques to image classifier.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Continued to prep the TeeSpring site.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 52/60 (8/18/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Stuck trying to figure out how to do one of the following: 
        <ul>
          <li>Federate an ImageFolder dataset</li>
          <li>Convert an ImageFolder dataset into a format that can be federated</li>
        </ul>
        See <a href="https://secureprivataischolar.slack.com/archives/CJW72L5D2/p1566158598309000">link</a> for post with code snippet.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Continued to prep the TeeSpring site.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>

<br>
<h3>Day 53/60 (8/19/2019)</h3>
<hr>
<h4>Step 1:</h4>
<p>pledge made 6/27/2019</p>
<h4>Step 2:</h4>
<ul>
  <li>Final Project:
    <ul>
      <li>Working on building a new dataset that can be federated.</li>
    </ul>
  </li>
  <li>Progress continues on Project T-Shirt:  
    <ul>
      <li>Continued to prep the TeeSpring site.</li>
      <li>Monitoring the Slack channel for questions. Much thanks to the team for their troubleshooting and support for those interested in the project.</li>
      <li>Working with @Yaak, @Sarah Majors, and @LauraT on TeeSpring logistics.</li>
      <li>Shout-out to the team: @Sarah Majors, @Parth Hiren Shah, @LauraT, @KT, @Yaak :all_the_things: Y'all are awesome!</li>
    </ul>
  </li>
  <li>Archived #60DaysofUdacity activity in README of the <a href="https://github.com/ewotawa/secure_private_ai">GitHub repo</a> where I am storing my notebooks: <a href="https://github.com/ewotawa/secure_private_ai">https://github.com/ewotawa/secure_private_ai</a></li> 
</ul>
<h4>Step 3:</h4>  
<p>hello world!</p>
<h4>Step 4:</h4>
<p>Shout-out to my #60daysofUdacity friends! You got this! :bananadance:</p>
<ul>
  <li>@Anju Mercian</li>
  <li>@cibaca</li>
  <li>@Aleksandra Deis</li>
  <li>@Livia Lohanda</li>
  <li>@Irene0000</li>
  <li>@Dushyant Pathak</li>
  <li>@Dishin Goyani</li>
</ul>
